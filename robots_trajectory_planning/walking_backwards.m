...
if t>10
    a=u(17);
    b=u(18);
    plot(10*t,a/100,'r.'),hold on
    plot(10*t,b/100,'g.'),hold on
    title('大腿关节力矩变化图') 
    xlabel('TIME')
    ...
    switch Order
        case 1
            if Swing_Flag==0%adjust cog
                ...
                cog_offset  = get_cross_cog_margin( next_foot_point, RF, LB, RB, Order );
        ...
        case 2
            if Swing_Flag==0%adjust cog
                ...
                cog_offset  = get_cross_cog_margin( LF, next_foot_point, LB, RB, Order );
                if ~isequal( cog_offset, [0, 0] )
                    [A_Adj, B_Adj, C_Adj, D_Adj] = cog_adjust_single( LF, RF, LB, RB, cog_offset );
                    sys(1:12) = get_angle_through_position( A_Adj, B_Adj, C_Adj, D_Adj );
                    return
